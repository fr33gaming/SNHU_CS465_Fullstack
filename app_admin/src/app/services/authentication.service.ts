...
export class AuthenticationService {
  ...
  // the key for our token 'tavlr-token'
  public getToken() : string {
    let out: any;
    out = this.storage.getItem('travlr-token');

    // Make suer we return a string even if we don't have a token
    if(!out)
    {
      return '';
    }
  ...
  // NOTE: For this application we have decided that we will name
  // the ey four token 'travlr-token'
  ...
  // is still valid. Even if we have a token we will still have to
  ...
  // Because that method returns an observable, we subscribe to the
  // result and only process when the Observable condition is satisfied
  // Uncomment the two following console.log messages for additional debugging
  // information. Please Note: This method is nearly identical to the 
  // login methid because the behavior of the API logs a new user in 
